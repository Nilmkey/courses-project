  ...
  useEffect(() => {
    const loadProgress = async () => {
      try {
        ...
        const progressDetail = await progressApi.getFullCourseProgress(
          course._id,
        );
        ...
        for (const section of sections) {
          for (const lesson of section.lessons) {
            ...
            const completedBlocksCount =
              lessonProgressData?.completedBlocksCount || 0;
  ...
  const completeBlock = useCallback(
    async (lessonId: string, blockId: string, quizAnswers?: IQuizAnswer[]) => {
      ...
      setLessonProgress((prev) => {
        ...
        const newCompletedBlocks = Math.min(
      ...
      // Отправляем на сервер (без await, чтобы не блокировать UI)
      progressApi
        .markBlockComplete(lessonId, courseId, {
          courseId,
          blockId,
          quizAnswers,
        })
        .catch((error) => {
          ...
          if (
            ...
            console.log(
              "Конфликт версий MongoDB (ожидаемое поведение):",
              blockId,
            );
          } else {
