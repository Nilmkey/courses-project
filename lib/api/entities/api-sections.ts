...
export const sectionsApi = {
  ...
  getByCourse: (courseId: string) =>
    api.get<SectionsListResponse>(`/v1/sections/course/${courseId}`, undefined, true),

  /**
   * Создать новую секцию
   */
  create: (courseId: string, data: CreateSectionData) =>
    api.post<SectionResponse>(`/v1/sections/${courseId}`, data, undefined, true),

  /**
   * Обновить секцию
   */
  update: (sectionId: string, data: UpdateSectionData) =>
    api.patch<SectionResponse>(`/v1/sections/${sectionId}`, data, undefined, true),
...
export const lessonsApi = {
  ...
  getBySection: (sectionId: string) =>
    api.get<LessonsListResponse>(`/v1/lessons/section/${sectionId}`, undefined, true),
