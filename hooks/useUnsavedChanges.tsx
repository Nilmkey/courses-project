...
function UnsavedWarningToast({ message, onConfirm, onCancel }: UnsavedWarningToastProps) {
  return (
    <div className="flex flex-col gap-3 p-4 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl max-w-sm">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          <span className="font-bold text-slate-800 dark:text-slate-100 block">{message}</span>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">Все несохраненные изменения будут утеряны</p>
        </div>
      </div>
